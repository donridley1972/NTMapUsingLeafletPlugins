   ...
   MEMBER('web76.clw')                                     ! This is a MEMBER module
                     ...
                     MAP
                       INCLUDE('WEB76007.INC'),ONCE        !Local module procedure declarations
                     END
...
ThisView            &View
View:PatrolArea       View(PatrolArea)
                      Project(Pat:Guid)
                      Project(Pat:Name)
                    END ! of ThisView
!-- drop
...
FilesErrorOnOpen  StringTheory
  CODE
  GlobalErrors.SetProcedureName('BrowsePatrolArea')
  ...
  If p_web.site.ContentBody <> '' and lower(p_web.GetValue('_cb_')) = lower('BrowsePatrolArea') then loc:ContentBody=1.
  if loc:RandomBrowseId = '' then loc:RandomBrowseId = p_web.Crc32(clip(loc:parent) & ' BrowsePatrolArea' & p_web.GetValue('_parentRow_')).
  ...
  If loc:parent <> '' and loc:ParentRow <> ''
    loc:divname = lower(clip(loc:parent) & net:PARENTSEPARATOR & 'BrowsePatrolArea' & net:PARENTSEPARATOR & lower(loc:parentRow))
    p_web.GetBrowseValue(p_web.GetValue('_parentRow_')) ! need to reget, so it's not lowered
  elsif loc:parent <> ''
    loc:divname = lower(clip(loc:parent) & net:PARENTSEPARATOR & 'BrowsePatrolArea')
  else
    loc:divname = lower('BrowsePatrolArea')
  end
...
loc:percentage  long
  code
  loc:percentage = p_web.GSV('Export_BrowsePatrolArea_PercentageComplete')
...
Percentage      Long
  code
  ExcelExport.SetWorkSheet('PatrolArea')
  ExcelExport.WithWorkSheet('PatrolArea')
  do OpenFilesB
  If p_web.sqlsync then p_web.SqlWait(p_web.SqlName).
  Open(ThisView)
  ThisView{prop:order} = p_web.GetSessionValue('BrowsePatrolArea_CurrentOrder_' & loc:RandomBrowseId)
  ThisView{prop:filter} = p_web.GetSessionValue('BrowsePatrolArea_CurrentFilter_' & loc:RandomBrowseId)
  ExpectedRecords = records(PatrolArea)
  ...
  ExcelExport.SetCell(rowNumber,1,p_web.translate('Patrol Area'),'d151')
  ...
  LOOP
    ...
    p_web.SetSessionValue('Export_BrowsePatrolArea_PercentageComplete',percentage)
    ExcelExport.SetRow(rowNumber)
    ColumnNumber += 1
    If DateColumn[ColumnNumber]
      ExcelExport.SetCell(rowNumber,ColumnNumber,p_web.FormatValue(Pat:Name,'@D010-B'),'d' &  DateColumn[ColumnNumber],,'DateTime')
      If 12 > ColumnWidth[ColumnNumber] then ColumnWidth[ColumnNumber] = 12.
    ElsIf TimeColumn[ColumnNumber]
      ExcelExport.SetCell(rowNumber,ColumnNumber,'1899-12-31T' & p_web.FormatValue(Pat:Name,'@T04B'),'t' &  TimeColumn[ColumnNumber],,'DateTime')
      If 12 > ColumnWidth[ColumnNumber] then ColumnWidth[ColumnNumber] = 12.
    Else
      ExcelExport.SetCell(rowNumber,ColumnNumber,Pat:Name)
      If Len(Clip(Pat:Name)) > ColumnWidth[ColumnNumber] then ColumnWidth[ColumnNumber] = Len(Clip(Pat:Name)).
    End
    rowNumber += 1
  End
  ...
  End
  ExcelExport.Save()
  p_web.SetSessionValue('Export_BrowsePatrolArea_PercentageComplete',100)
  p_web.HeaderDetails.ContentDisposition = 'attachment; filename="' & clip('PatrolArea') & '.xml"'
...
parent_       string(100)
  code
  If loc:parent
    p_web.AlertParent('BrowsePatrolArea')
  End
...
loc:PopupName    String(252)
  code
    ...
    if loc:CallPopups = 1 or loc:CallPopups = 6 ! browse not embedded on form so include popup divs and scripts for this browse
      if loc:CallPopups = 1 then loc:PopupName = 'BrowsePatrolArea' else loc:PopupName = clip(loc:parent) & net:PARENTSEPARATOR & 'BrowsePatrolArea' .
      if p_web.GetPreCall('popup_' & clip(loc:PopupName)) = 0
        ...
        p_web.DivHeader(clip(loc:PopupName),p_web.combine(p_web.site.style.browsediv,),,'BrowsePatrolArea',,1)
        ...
        End
        p_web.jQuery('#' & lower('popup_BrowsePatrolArea_div'),'dialog',loc:options,'.removeClass("nt-hidden")') !& |
...
ClearBrowse  Routine
  p_web.ClearBrowse('BrowsePatrolArea_' & loc:randomBrowseId)
...
CallBrowse  Routine
  ...
  If loc:FrontLoading = 0
    ...
    do Children
    If p_web.site.ContentBody <> '' and lower(p_web.GetValue('_cb_')) = lower('BrowsePatrolArea')
      p_web.Divfooter()
    End
    do ClosingScripts
    do SendPacket
...
LookupAbility  Routine
  If p_web.IfExistsValue('Lookup_Btn')
    ...
    p_web.PrimeForLookup(PatrolArea,Pat:GuidKey,loc:vorder)
    If False
    ElsIf (loc:vorder = 'PAT:NAME') then p_web.SetValue('BrowsePatrolArea_sort','1')
    End
  End
  If p_web.IfExistsValue('LookupField') and p_web.GetValue('BrowsePatrolArea:parentIs') <> 'Browse'
    loc:selecting = true
    p_web.StoreValue('BrowsePatrolArea:LookupField','LookupField')
  elsif p_web.Ajax > 0 and p_web.IfExistsSessionValue('BrowsePatrolArea:LookupField') > 0
    loc:selecting = true
  else
    p_web.DeleteSessionValue('BrowsePatrolArea:LookupField')
    loc:selecting = false
  End
...
loc:options             StringTheory ! options for jQuery calls
  ...
  If (loc:popup = 0 or p_web.site.FrontLoaded = 1)
    ...
    If p_web.GetPreCall('UpdatePatrolArea') = 0 then UpdatePatrolArea(p_web,Net:Web:Popup).  !Form Procedure
    p_web.SetValue('_CallPopups',0)
    p_web.PopEvent()
  End

SetFormAction  Routine
  loc:formaction = 'UpdatePatrolArea'
...
Heading  Routine
  If band(loc:stage,net:web:GenerateHeadings + net:web:UpdateHeadings)
    ...
    Else
      loc:Heading = p_web.Translate('Patrol Area',(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0))
    End
    do SendPacket
...
SetSortHeader  Routine
  ...
  Of upper('Pat:Name')
    ...
    p_web.SetSessionValue('BrowsePatrolArea_LocatorPic_' & clip(loc:RandomBrowseId),'@s20')
    loc:LocatorType = Net:Search     ! default locator type for this browse
  End

SetVorder  Routine
  loc:vordernumber    = p_web.RestoreValue('BrowsePatrolArea_sort_' & loc:RandomBrowseId,net:DontEvaluate)
  ThisView &= View:PatrolArea
  If loc:vordernumber = 0
    loc:vordernumber = 1
  End
  p_web.SetSessionValue('BrowsePatrolArea_sort_' & loc:RandomBrowseId ,loc:vordernumber)
  Loc:SortDirection = choose(loc:vordernumber < 0,-1,1)
  case abs(loc:vordernumber)
  of 1
    loc:vorder = Choose(Loc:SortDirection=1,'UPPER(Pat:Name)','-UPPER(Pat:Name)')
    Loc:LocateOnFields = 'Pat:Name'
    Loc:LocateField = 'Pat:Name'
    Loc:LocatorCase = 0
  end
  if loc:vorder = ''
    loc:vorder = '+UPPER(Pat:Guid)'
  end
...
loc:options             StringTheory ! options for jQuery calls
  ...
  If loc:selecting = true
    ...
    loc:selectaction = p_web.FormSettings.ParentPage !p_web.GetSessionValue('BrowsePatrolArea:LookupFrom')
    p_web.GetSettings(p_web.GetSessionValue('Push1'))
    loc:CancelAction = p_web.FormSettings.ParentPage !p_web.GetSessionValue('BrowsePatrolArea:LookupFrom')
  End !Else
...
BrowseTable  routine
  ...
  if loc:ParentSilent = false
    ...
    If p_web.RequestJson = 0
      packet.append(p_web.BrowseTableRowStart('',' nt-browse-row-header browsepatrolarea-row-header','browse-header-row','',loc:LayoutMethod))
    Else
    ...
    If p_web.RequestJson
      packet.append('"cells":[')
    End
            packet.append(p_web.CreateSortHeader(loc:vordernumber,'1','BrowsePatrolArea',p_web.Translate('Name',(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),'Click here to sort by Name',,,p_web.Combine(),1, loc:columns+1 ,,,0,loc:Sorting,'String','nt-left',loc:LayoutMethod,' title="'&p_web._jsok('Click here to sort by Name')&'"',loc:CellStarted,0 + 1))
            loc:CellStarted = false
            do AddPacket
            loc:columns += 1
        If loc:Selecting = 0
            If p_web.RequestJSON then packet.append(',').
            packet.append(p_web.CreateSortHeader(loc:vordernumber,'2','BrowsePatrolArea',p_web.Translate(,(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),,,,p_web.Combine(),1, loc:columns+1 ,,,1,loc:Sorting,'Button','nt-left',loc:LayoutMethod,'',loc:CellStarted,0 + 1))
            loc:CellStarted = false
            do AddPacket
            loc:columns += 1
        End ! Selecting
        If loc:Selecting = 0
            If p_web.RequestJSON then packet.append(',').
            packet.append(p_web.CreateSortHeader(loc:vordernumber,'3','BrowsePatrolArea',p_web.Translate(,(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),,,,p_web.Combine(),1, loc:columns+1 ,,,1,loc:Sorting,'Button','nt-left',loc:LayoutMethod,'',loc:CellStarted,0 + 1))
            loc:CellStarted = false
            do AddPacket
            loc:columns += 1
        End ! Selecting
        If loc:Selecting = 0
            If p_web.RequestJSON then packet.append(',').
            packet.append(p_web.CreateSortHeader(loc:vordernumber,'4','BrowsePatrolArea',p_web.Translate(,(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),,,,p_web.Combine(),1, loc:columns+1 ,,,1,loc:Sorting,'Button','nt-left',loc:LayoutMethod,'',loc:CellStarted,0 + 1))
            loc:CellStarted = false
            do AddPacket
            loc:columns += 1
        End ! Selecting
        If loc:Selecting = 1
            If p_web.RequestJSON then packet.append(',').
            packet.append(p_web.CreateSortHeader(loc:vordernumber,'5','BrowsePatrolArea',p_web.Translate(,(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),,,,p_web.Combine(),1, loc:columns+1 ,,,1,loc:Sorting,'Button','nt-left',loc:LayoutMethod,'',loc:CellStarted,0 + 1))
            loc:CellStarted = false
            do AddPacket
            loc:columns += 1
        End ! Selecting
    ...
    if loc:found = 0
      If p_web.RequestJson = 0
        ...
        packet.append(p_web.Translate('no Patrol Area'))
        packet.append(p_web.BrowseTableCellEnd('',loc:LayoutMethod))
        packet.append(p_web.BrowseTableRowEnd('',loc:LayoutMethod))
      Else
        packet.append('"rows":[ {{"row":{{"cells":[{{"cell":{{"class":"'&p_web.combine(p_web.site.style.BrowseEmpty,)&'",content:"'&p_web.Translate('no Patrol Area')&'"}}]}}]')
      End
      do AddPacket
    ...
    p_web.SetSessionValue('BrowsePatrolArea_prop:Order_' & loc:RandomBrowseId,ThisView{prop:order})
    p_web.SetSessionValue('BrowsePatrolArea_prop:Filter_' & loc:RandomBrowseId,ThisView{prop:filter})
    Close(ThisView)
    If p_web.sqlsync then p_web.SqlRelease(p_web.SqlName).
    ...
    do SendPacket
  End
  packet.append(p_web.DivFooter(Net:NoSend,'BrowsePatrolArea_table_div',p_web.RequestJson)) ! Table Div
  do SendPacket
  If loc:FrontLoading = 0
    do SendPacket
  End
  ...
  if(loc:FileLoading=Net:PageLoad)
    p_web.SetSessionValue('BrowsePatrolArea_FirstValue_' & loc:RandomBrowseId,loc:firstvalue)
    p_web.SetSessionValue('BrowsePatrolArea_LastValue_' & loc:RandomBrowseId,loc:lastvalue)
  End
  do CloseFilesB
...
loc:viewoptions  Long
  ...
  End
  If Instring('pat:guid',lower(loc:vorder),1,1) = 0 !and PatrolArea{prop:SQLDriver} = 1
    loc:vorder = Choose(loc:vorder='','UPPER(Pat:Guid)',clip(loc:vorder) & ',' & 'UPPER(Pat:Guid)')
  End
  Loc:Selected = Choose(p_web.IfExistsValue('Pat:Guid'),p_web.GetValue('Pat:Guid'),p_web.GetSessionValue('Pat:Guid'))
  ...
  if loc:filterwas <> p_web.GetSessionValue('BrowsePatrolArea_Filter_' & loc:RandomBrowseId)
    p_web.SetSessionValue('BrowsePatrolArea_FirstValue_' & loc:RandomBrowseId,'')
    p_web.SetSessionValue('BrowsePatrolArea_Filter_' & loc:RandomBrowseId,loc:filterwas)
  end
  do FindAnchor
  p_web.SetSessionValue('LocatorField:' & clip(loc:divname),lower(left(Loc:LocateField)))
  p_web.SetView(ThisView,PatrolArea,Pat:GuidKey,loc:PageRows,'BrowsePatrolArea',left(Loc:LocateOnFields),left(Loc:LocateField),loc:FileLoading,loc:LocatorType,clip(loc:LocatorValue),Loc:SortDirection,loc:ViewOptions,Loc:FillBack,Loc:Direction,loc:NextDisabled,loc:PreviousDisabled,Loc:LocatorCase,loc:RandomBrowseId) ! loc:pagerows is not used in here.
  p_web.SetSessionValue('BrowsePatrolArea_CurrentOrder_' & loc:RandomBrowseId,ThisView{prop:order})
  p_web.SetSessionValue('BrowsePatrolArea_CurrentFilter_' & loc:RandomBrowseId,ThisView{prop:filter})
  If loc:LocatorBlank = 0 or Loc:LocatorValue <> '' or Loc:LocatorType = Net:Date or loc:LocatorType = Net:Position or loc:LocatorType = Net:NoLocator
    ...
    Loop
      ...
      If(loc:FileLoading=Net:PageLoad)
        ...
        End
      End
      loc:viewstate = p_web.escape(p_web.Base64Encode(clip(Pat:Guid)))
      do BrowseRow
      loc:recordsCount += 1
    end ! loop
    loc:InView = 0
    ...
    If p_web.RequestJson = 0
      packet.append(p_web.BrowseTableRowStart('',' nt-browse-locator-row browsepatrolarea-locator-row','browse-locator-row','',loc:LayoutMethod))
      packet.append(p_web.BrowseTableCellStart('',' nt-browse-locator-cell browsepatrolarea-locator-cell',1,1,1,1,'browse-locator-cell','',loc:LayoutMethod))
...
GenerateUpdateButtonsAbove  Routine
  If (loc:UpdateButtonPosition=Net:Above or loc:UpdateButtonPosition=Net:Both) and band(loc:stage,net:web:GenerateUpdates)
    ...
    If (loc:selecting = 0 or loc:popup)
      If p_web.CanCall('UpdatePatrolArea',0,,) = net:ok
        If loc:viewonly = 0
          packet.append(p_web.CreateStdBrowseButton(Net:Web:InsertButton,'BrowsePatrolArea',,     ,loc:FormPopup,'UpdatePatrolArea',))
          do SendPacket
        End
      End
    End
        If p_web.site.DefaultExport
          packet.append(p_web.CreateStdBrowseButton(Net:Web:ExportButton,'BrowsePatrolArea',,,,,) & p_web.CRLF)  !b
        End
    ...
    do SendPacket
      packet.append('<div id="BrowsePatrolArea_select_a" class="'&p_web.combine(p_web.site.style.BrowseSelectButtonSet,)&'">')
    If loc:selecting = 1
      packet.append(p_web.CreateStdBrowseButton(Net:Web:SelectButton,'BrowsePatrolArea',,,loc:popup,,))
      do SendPacket
    End
    If loc:selecting = 1 !and loc:parent = ''
      packet.append(p_web.CreateStdBrowseButton(Net:Web:BrowseCancelButton,'BrowsePatrolArea',,,loc:popup,,))
      do SendPacket
    End
    packet.append('</div>' & p_web.CRLF)
    If p_web.site.UseSelectButtonSet
      loc:options.Free(True)
      p_web.jQuery('#' & 'BrowsePatrolArea_select_a','controlgroup',loc:options)
...
GenerateUpdateButtonsBelow  routine
  If (loc:UpdateButtonPosition=Net:Below or loc:UpdateButtonPosition=Net:Both) and band(loc:stage,net:web:GenerateUpdates)
    ...
    End ! If p_web.site.UseUpdateButtonSet
    do SendPacket
      packet.append('<div id="BrowsePatrolArea_select_b" class="'&p_web.combine(p_web.site.style.BrowseSelectButtonSet,)&'">')
    If loc:selecting = 1
      packet.append(p_web.CreateStdBrowseButton(Net:Web:SelectButton,'BrowsePatrolArea',,,loc:popup,,))
      do SendPacket
    End
    ...
    If p_web.site.UseSelectButtonSet
      loc:options.Free(True)
      p_web.jQuery('#' & 'BrowsePatrolArea_select_b','controlgroup',loc:options)
    End ! If p_web.site.UseSelectButtonSet
    do SendPacket
    do SendPacket
...
GenerateNavButtonsAbove  routine
  If loc:FileLoading=Net:PageLoad and band(loc:stage,net:web:GenerateNav) and (loc:NavButtonPosition=Net:Above or loc:NavButtonPosition=Net:Both)
      ...
      packet.append(p_web.CreateStdButton('button',Net:Web:FirstButton,,,,,,loc:previousdisabled,,'BrowsePatrolArea')) !p1
      packet.append(p_web.CreateStdButton('button',Net:Web:PreviousButton,,,,,,loc:previousdisabled,,'BrowsePatrolArea')) !p2
      packet.append(p_web.CreateStdButton('button',Net:Web:NextButton,,,,,,loc:nextdisabled,,'BrowsePatrolArea')) !p3
      packet.append(p_web.CreateStdButton('button',Net:Web:LastButton,,,,,,loc:nextdisabled,,'BrowsePatrolArea')) !p4
      packet.append('</div>' & p_web.CRLF)
...
BrowseRow  routine
  ...
  If loc:eip = 0
    If(loc:InView)
      loc:field = p_web.AddBrowseValue('BrowsePatrolArea_' & loc:RandomBrowseId,'PatrolArea',Pat:GuidKey,ThisView)
    Else
      loc:field = p_web.AddBrowseValue('BrowsePatrolArea_' & loc:RandomBrowseId,'PatrolArea',Pat:GuidKey)
    End
  End
  p_web._thisrow = p_web.nocolon('Pat:Guid')
  ...
  if loc:eip = 0
    if Loc:LocatorValue <> '' and loc:ActualSelection = ''
      ...
      loc:checked = 'checked'
      do SetSelection
    elsif loc:ActualSelection = '' and Pat:Guid = p_web.GetSessionValue('Pat:Guid')
       loc:checked = 'checked'
       do SetSelection
    elsif loc:selecting = 1
      loc:checked = Choose(p_web.getsessionvalue(p_web.GetSessionValue('BrowsePatrolArea:LookupField')) = Pat:Guid and loc:ActualSelection = '','checked','')
      if loc:checked <> '' then do SetSelection.
    else
      loc:checked = Choose((Pat:Guid = loc:selected) and loc:ActualSelection = '','checked','')
      if loc:checked <> '' then do SetSelection.
    end
...
DataDo  String(100)
  ...
  End
  If p_web.RequestJson = 0
    packet.append(p_web.BrowseTableRowStart('',' ' & clip(loc:rowstyle) & ' nt-browse-row-data browsepatrolarea-row-data','browse-row',loc:RecordExtra.GetValue() & ' ' & dataDo,loc:LayoutMethod,))
  Else
...
StartRow  Routine
  loc:rowcount += 1
  TableQueue.Idx = loc:field !p_web.AddBrowseValue('BrowsePatrolArea','PatrolArea',Pat:GuidKey,ThisView)
  TableQueue.Id[1] = Pat:Guid
  Loc:cellStarted = false
...
FirstInCell                     long
  code
    do SetFormAction
    loc:options.Free(True)
    p_web.SetOption(loc:options,'procedure',lower('BrowsePatrolArea'))
    ...
    p_web.SetOption(loc:options,'lookupField',p_web.GetSessionValue('BrowsePatrolArea:LookupField'))
    If p_web.CanCallAddSec() = net:ok
      p_web.SetOption(loc:options,'addsec','BrowsePatrolArea') !b
    End
...
parent_  string(252)
  code
  parent_ = p_web.SetParent(loc:parent,'BrowsePatrolArea')
! ----------------------------------------------------------------------------------------
CallClicked  Routine
  p_web.SetSessionValue('Pat:Guid',p_web.GetValue('Pat:Guid'))
...
loc:hash  string(Net:HashSize)
  code
  do OpenFilesB
  Pat:Guid = p_web.GetSessionValue('Pat:Guid')
  loc:result = p_web.GetFile(PatrolArea,Pat:GuidKey)
  if loc:result = 0
    loc:hash = p_web.GetBrowseHash('BrowsePatrolArea_' & loc:RandomBrowseId,'PatrolArea',PatrolArea,Pat:GuidKey ,Pat:Guid)
    if loc:hash
      p_web.GetBrowseValue(loc:hash) ! primes p_web.viewpos for use in GetView
    end
  end
  loc:result = p_web.GetView(ThisView,'BrowsePatrolArea',loc:RandomBrowseId)
  loc:InView = 1
  loc:eip = 1
  loc:viewstate = p_web.escape(p_web.Base64Encode(clip(Pat:Guid)))
  do BrowseRow
...
loc:FilterA         StringTheory
  code
    ...
    Else ! default settings for browse column
      ...
      End
      packet.append(p_web.DivHeader('BrowsePatrolArea_Pat:Name_'&Pat:Guid,' nt-flex nt-browse-data',net:crc,,loc:extra))
      packet.append( p_web.CreateHyperLink(p_web._jsok(Left(p_web.FormatValue(Pat:Name,'@s20')),(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0)),,,,loc:javascript,,,(Net:HtmlOk*0)+(Net:UnsafeHtmlOk*0),,,'BrowsePatrolArea'))
    End
    packet.append(p_web.DivFooter(Net:NoSend))
    if loc:eip = 1
      do SendPacket
...
loc:FilterA         StringTheory
  code
    ...
    Else ! default settings for browse column
      loc:extra = ''
      packet.append(p_web.DivHeader('BrowsePatrolArea_Copy_'&Pat:Guid,' nt-flex nt-browse-data',net:crc,,loc:extra))
          If p_web.CanCall('UpdatePatrolArea',0,,) = net:ok
            If loc:viewonly = 0
               packet.append(p_web.CreateStdBrowseButton(Net:Web:SmallCopyButton,'BrowsePatrolArea',loc:field,false,loc:FormPopup,'UpdatePatrolArea')  & p_web.CRLF)
            End
          End
    End
    packet.append(p_web.DivFooter(Net:NoSend))
...
loc:FilterA         StringTheory
  code
    ...
    Else ! default settings for browse column
      loc:extra = ''
      packet.append(p_web.DivHeader('BrowsePatrolArea_Change_'&Pat:Guid,' nt-flex nt-browse-data',net:crc,,loc:extra))
          If p_web.CanCall('UpdatePatrolArea',0,,) = net:ok
            If loc:viewonly = 0
               packet.append(p_web.CreateStdBrowseButton(Net:Web:SmallChangeButton,'BrowsePatrolArea',loc:field,false,loc:FormPopup,'UpdatePatrolArea')  & p_web.CRLF)
            End
           End
    End
    packet.append(p_web.DivFooter(Net:NoSend))
...
loc:FilterA         StringTheory
  code
    ...
    Else ! default settings for browse column
      loc:extra = ''
      packet.append(p_web.DivHeader('BrowsePatrolArea_Delete_'&Pat:Guid,' nt-flex nt-browse-data',net:crc,,loc:extra))
          If p_web.CanCall('UpdatePatrolArea',0,,) = net:ok
            If loc:viewonly = 0
              packet.append(p_web.CreateStdBrowseButton(Net:Web:SmallDeleteButton,'BrowsePatrolArea',loc:field,false,loc:FormPopup,'UpdatePatrolArea')  & p_web.CRLF)
            End
          End
    End
    packet.append(p_web.DivFooter(Net:NoSend))
...
loc:FilterA         StringTheory
  code
    ...
    Else ! default settings for browse column
      loc:extra = ''
      packet.append(p_web.DivHeader('BrowsePatrolArea_Select_'&Pat:Guid,' nt-flex nt-browse-data',net:crc,,loc:extra))
        packet.append(p_web.CreateStdBrowseButton(Net:Web:SmallSelectButton,'BrowsePatrolArea',loc:field,,loc:popup))
    End
    packet.append(p_web.DivFooter(Net:NoSend))
    if loc:eip = 1
      do SendPacket
